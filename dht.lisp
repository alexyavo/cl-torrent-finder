...
(defun get-torrent-peers (info-hash &key (min-peers 10))
  "Returns requested amount of peers that have info-hash"
  (let ((peers (make-set-collection))
        (nodes (make-set-collection :initial-items +well-known-nodes+)))
    (while (and (> (items-count nodes) 0)
                (< (items-count peers) min-peers))
      (on-query-success (node-peers (get-peers (node-address (pop-item nodes)) info-hash))
        (add-items peers (peer-values node-peers))
        (add-items nodes (peer-nodes node-peers))))
    (get-items peers)))
